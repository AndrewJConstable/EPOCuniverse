...
#
#setGeneric("initialiseTrial", function(.Object, trial="Trial", config="list") standardGeneric("initialiseTrial"))
setMethod("initialiseTrial", signature(element="KrillEnvironment", universe="Universe"), 
    function(element, universe) {
		...
		if (!file.exists(fileName) | getAttribute(element, "ReplaceFile")) {
			...
			writeFileConnection(element, "Trial", "Year", asCSVCharacter(getSpatial(universe, "polygonNames")[getPolygons(element)]), sep=",", conn=fileConn)
			...
			if (!require("MASS", quietly=TRUE)) {
				epocErrorMessage(element, "MASS package required for mvrnorm function in KrillEnv initialiseTrial method.  Please install!", halt=TRUE)
			}
