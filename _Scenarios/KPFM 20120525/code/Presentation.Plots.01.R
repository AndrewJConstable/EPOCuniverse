...
ProbQuantile<-0.5
PlotTrial<-NA  # if NA then plot quantile across all trials
...
for (Ne in 1:Nelmnts){
  ...
  # plot value for a given trial or quantile from trials
  if (!is.na(PlotTrial)){
    PlotData<-PlotData[PlotData[,1]==PlotTrial,c(2,3)]
    } else {
    PD<-NULL
    for (y in 1:YearsN){
      for (tm in 1:TimesN){
...
PlotData<-read.csv(paste(RootDir,PlotElmnts[[Eno]]$State_Stage,sep=""), header = FALSE)
             ...
             # Col 8 =  Health
PlotData<-PlotData[PlotData[,1]==Trial,]
