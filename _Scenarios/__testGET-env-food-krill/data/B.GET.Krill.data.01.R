...
Krill$Stage             <- list(StageN         = Krill$StageN         # "larvae", "recruited juveniles", "adults"
                               ...
                               ,StageStr       = matrix(rep(c(0.0,0.55,0.45),Krill$polygonsN),ncol=Krill$polygonsN)      # list by polygon TO BE PROVIDED
                               ,StageSize      = matrix(   # established as a matrix: cols=polygon, rows=stage
		                            ...
		                            ,StageCondR    = matrix(rep(c(0,0,0.1), Krill$polygonsN),ncol=Krill$polygonsN)
...
MigrateMat<-matrix(rep(0,PPfood$polygonsN*PPfood$polygonsN),ncol=PPfood$polygonsN)
for(pn in 1:PPfood$polygonsN) MigrateMat[pn,pn]<-1
...
Krill$data.advanceStage <- rep(list(  # list by polygon  # uses function advanceStageWtdBMS
                           ...
                           # NOTE = make sure each row sums to 1
              ...
              AllTimes  = list( # by predator stage - if NULL then no consumption by that stage
                    ...
                    #-------------------------
                     ,Juvenile = list(
                         ...
                         ,Prey = list(
                            PPfood = list(
                                    CalcByRate           = TRUE # if TRUE then calc consumption as exponential rate per day rather than fixed quantum per year
                                   ,PerCapita            = 35  # maximum per g consumption per year
                                   ,PropInDiet           = 1  # proportion of this prey in diet
                                   ,Holling_q            = 1
                                   ,Holling_D            = 4E-3
                                   ,UseFoodPerCapitaForD = FALSE  # If FALSE then density of prey calculated as prey/polygon area, if TRUE then prey per food
                                   ,Holling_units        = 2
...
Krill$data.AllocateConsumption<-list(Summer = list(
                                        ...
                                        ,dMetab   = list(Larvae    = 0.0,    # per mass metabolic requirements per year
                                                         Juveniles = 0.76,
                                                         Adults    = 1.25)
                                        ,dReprod  = list(Larvae    = list(pR = 0,  cR = 0,Rmax = 0),
                                                         Juveniles = list(pR = 0,  cR = 0,Rmax = 0),
                                                         Adults    = list(pR = 0.9,cR = 3.76,Rmax = 0.95) # average Rmax = RmaxFemale*SexRatio*MaturityOgive
                                                         ) # end dReprod
                                        ,dGrowth  = list(Larvae    = list(cG = 0, Gmax = 0),
                                                         Juveniles = list(cG = 31.5, Gmax = 2.8),
                                                         Adults    = list(cG = 12.0, Gmax = 0.45))
                                        ,Starve   = list(Larvae    = list(costShrink=0,ShrinkMax = 0),
                                                         Juveniles = list(costShrink=30.0,ShrinkMax = 0.071),
                                                         Adults    = list(costShrink=12.0,ShrinkMax = 0.043))
                                        ...
                                        ,Cpp      = list( # list by stage with a matrix (rows = origin, cols = destination) for immigration to destination polygons of
                                                      ...
                                                      Larvae    = matrix(rep(1.0,Krill$polygonsN*Krill$polygonsN),ncol=Krill$polygonsN)  # 1 =  all per capita food for immigrants comes from origin polygon
                                                     ,Juveniles = matrix(rep(1.0,Krill$polygonsN*Krill$polygonsN),ncol=Krill$polygonsN)
                                                     ,Adults    = matrix(rep(1.0,Krill$polygonsN*Krill$polygonsN),ncol=Krill$polygonsN)
                                                       ) # end Cpp
                                 ...
                                 ,Winter = list(
                                        ...
                                        ,FoodValue =  list(Larvae    = 0, # vector - food value for each prey type, in order of Prey in list of dependent elements
                                                           Juveniles = 1,
                                                           Adults    = 1)
                                        ,AllocateToReproduction = FALSE
                                        ,dMetab   = list(Larvae    = 0.0,    # per mass metabolic requirements per year
                                                         Juveniles = 0.25,   # one third summer
                                                         Adults    = 0.41)
                                        ,dReprod  = list(Larvae    = list(pR = 0,  cR = 0,Rmax = 0),
                                                         Juveniles = list(pR = 0,  cR = 0,Rmax = 0),
                                                         Adults    = list(pR = 0.9,cR = 3.76,Rmax = 0.95) # average Rmax = RmaxFemale*SexRatio*MaturityOgive
                                        ...
                                        ) # end winter (allocation to reproduction only occurs in summer
...
Krill$data.Spawn<-list(EggProdn    = EggProdn, # matrix that distributes eggs from source to destination polygons, including cost of producing eggs from gonads
                       Cond.S      = rep(0.1,Krill$polygonsN), # size to be inherited to Stage 1 - vector for polygons
	...
	Winter = list(calday=dayFromDate(30,9),
		...
		actions=list(
                            ...
                            allocateConsumption = list(actionMethod    = Krill$FunctionsList$AllocateConsumption$actionMethod,
																											...
																											transAction     = NULL,
																											relatedElements = Krill$Consume$relatedElements,
																											dset            = Krill$data.AllocateConsumption[[2]]
			                                                ),
