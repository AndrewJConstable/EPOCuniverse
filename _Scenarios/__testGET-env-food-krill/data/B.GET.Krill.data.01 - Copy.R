...
Krill$data.advanceStage <- rep(list(  # by polygon  # uses function advanceStageWtdBMS
...
# make all matrices sum to 1
for(pn in 1:Krill$polygonsN) Krill$data.advanceStage[[pn]]<-Krill$data.advanceStage[[pn]]/sum(Krill$data.advanceStage[[pn]])
              ...
              AllTimes  = list( # by predator stage - if NULL then no consumption by that stage
                    ...
                    #-------------------------
                     ,Juvenile = list(
                         ...
                         ,Prey = list(
                            PPfood = list(
                                    PerCapita            = 1000  # maximum per capita consumption per year
                    ...
                    #-------------------------
                     ,Adult = list(
                         ...
                         ,Prey = list(
                            PPfood = list(
                                    PerCapita            = 5000  # maximum per capita consumption per year
                                   ,PropInDiet           = 1  # proportion of this prey in diet
                                   ,Holling_q            = 1
                                   ,Holling_D            = 10000
                                   ,Holling_units        = 2
...
Krill$data.AllocateConsumption<-list(Summer = list(
                                        ...
                                        ,FoodValue =  list(Larvae    = 1, # vector - food value for each prey type, in order of Prey in list of dependent elements
                                                           Juveniles = 1,
                                                           Adults    = 1)
                                        ...
                                        ,dMetab   = list(Larvae    = 0.0,    # per mass metabolic requirements per year
                                                         Juveniles = 0.01,
                                                         Adults    = 0.01)
                                        ,dReprod  = list(Larvae    = list(pR = 0,  cR = 1,Rmax = 0),
                                                         Juveniles = list(pR = 0,  cR = 1,Rmax = 0),
                                                         Adults    = list(pR = 0.5,cR = 1,Rmax = 1) # average Rmax = RmaxFemale*SexRatio*MaturityOgive
                                                         ) # end dReprod
                                        ,dGrowth  = list(Larvae    = list(cG = 1, Gmax = 100),
                                                         Juveniles = list(cG = 1, Gmax = 100),
                                                         Adults    = list(cG = 1, Gmax = 100))
                                        ,Starve   = list(Larvae    = list(costShrink=1,ShrinkMax = 0),
                                                         Juveniles = list(costShrink=1,ShrinkMax = 0.01),
                                                         Adults    = list(costShrink=1,ShrinkMax = 0.1))
