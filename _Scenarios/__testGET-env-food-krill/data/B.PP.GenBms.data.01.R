...
PPfood$Migration.data        <- list( # instantaneous rates of movement (as M)
                                      ...
                                      # one matrix per stage
                           Season1 = MigrateMat, # end season 1
                           Season2 = MigrateMat) # end season 1
...
PPfood$Init.density          <- c( 1 # APPA
)
PPfood$Init.density.multiplier <- 1E0
PPfood$StageN<-1
PPfood$Stage                 <- list(StageN = PPfood$StageN
                               ,StageStrUnits  = 2 # (1 = N, 2 = B)
                               ,StageStr  = matrix(rep(rep((1/PPfood$StageN),PPfood$StageN),PPfood$polygonsN),nrow=PPfood$StageN)
                               ,StageSize = matrix(rep(1,PPfood$StageN*PPfood$polygonsN),nrow=PPfood$StageN)
                               )
PPfood$Initialise            <- list(NULL)
PPfood$Transition.data       <- list(NULL)

PPfood$FoodUpdate.Data       <- list(cF = 1E4) # 100 thousand tonnes carbon
