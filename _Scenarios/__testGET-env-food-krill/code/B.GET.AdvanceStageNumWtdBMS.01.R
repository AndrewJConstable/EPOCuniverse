#B.GET.advanceStageNumWtdBMS.01<-function(
# add numbers in one stage to another stage and assume new size - leave zero in previous stage

if (!isGeneric("advanceStageNumWtdBMS"))
setGeneric("advanceStageNumWtdBMS", function(element, universe) standardGeneric("advanceStageNumWtdBMS"))
setMethod("advanceStageNumWtdBMS", signature(element="GET", universe="Universe"),
    function(
		element,
		...
		)     
    {
      ...
      Wt.Size   <- elemState$Stage[,pn] * elemState$Cond.S[,pn]
      Wt.Reprod <- elemState$Stage[,pn] * elemState$Cond.R[,pn]
      Wt.Health <- elemState$Stage[,pn] * elemState$Cond.H[,pn]

      elemState$Stage[,pn]  <- elemState$Stage[,pn]%*%transfer
      StageNot0 <- elemState$Stage[,pn]>0

       Trans<-(Wt.Size   %*% transfer)
      elemState$Cond.S[StageNot0,pn] <- Trans[StageNot0]/elemState$Stage[StageNot0,pn]
      elemState$Cond.S[(!StageNot0 | !(elemState$Cond.S[,pn]>0)),pn] <- 0

       Trans<-(Wt.Reprod   %*% transfer)
      elemState$Cond.R[StageNot0,pn] <- Trans[StageNot0]/elemState$Stage[StageNot0,pn]
      elemState$Cond.R[(!StageNot0 | !(elemState$Cond.S[,pn]>0)),pn] <- 0

       Trans<-(Wt.Health   %*% transfer)
      elemState$Cond.H[StageNot0,pn] <- Trans[StageNot0]/elemState$Stage[StageNot0,pn]
      elemState$Cond.H[(!StageNot0 | !(elemState$Cond.S[,pn]>0)),pn] <- 0
