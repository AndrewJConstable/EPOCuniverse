...
#  2.1 set up stage-polygon matrices
    ...
    if(StageQtyInPolygon>0){
       ...
       if (!is.null(Mortality)) {
                 ...
                 if(Records.Mort>0){
                    M.mort <- (-log(1-MortStage[,4] /StageQtyInPolygon))
                    Loss.mort<-M.mort
                    } else M.mort<- 0
                } else M.mort<-0
       ...
       if (!is.null(Emigration)) {
                 ...
                 Records.Em   <- nrow(EmStage)
                   if(Records.Em>0){
                      M.Em<-(-log(1-EmStage[,5]/StageQtyInPolygon))
                      Loss.em   <- M.Em
                      } else M.Em<-0
...
#     replace Emigration loss to matrix for use at end
        if(Records.Em>0) Emigration[(is.element(Emigration[,1],pn) & is.element(Emigration[,4],st)),5] <- Loss.em

#     replace Mortality loss to matrix for use at end
        if(Records.Mort>0) Mortality[(is.element(Mortality[,1],pn) & is.element(Mortality[,3],st) & !is.element(Mortality[,6],3)),4]<-Loss.mort
    ...
    HealthPrior<-elemState$Cond.H  # matrix (rows = stage, cols = polygon)
      ...
      # 3.3 By stage, update number, size, reproductive condition and health
          ...
          if(StQty[st]>0){
             ...
             NewStSize[st]   <- StSize[st]
             NewStReprod[st] <- StReprod[st]
             NewStHealth[st] <- StHealth[st]
             if(!is.null(Emigration)){
               ...
               #          based on remainder and origins of immigrants of size of numbers left and sizes of immigrants
                ...
                if(NewStQty[st]>0){
                  ...
                  NewStHealth[st] <- (Remainder*StHealth[st] +
                    ...
                    NewStReprod[st]<-0
                    NewStHealth[st]<-0
                    } # end if(NewStQty[st]>0)
                  } # end !is.null Emigration
              } # end    if(StQty[st]>0){
          ...
          elemState$Stage[,pn] <- if(elemState$Abundance[[elemState$StageStrUnits]][pn]>0) NewStQty/elemState$Abundance[[elemState$StageStrUnits]][pn] else
                                       rep((1/elemState$StageN),elemState$StageN)

          if(elemState$StageStrUnits==1) {
              elemState$Abundance[[2]][pn] <- sum(NewStQty*NewStSize)
              } else {
              elemState$Abundance[[1]][pn] <- sum(NewStQty[NewStSize>0]/NewStSize[NewStSize>0])
              }
