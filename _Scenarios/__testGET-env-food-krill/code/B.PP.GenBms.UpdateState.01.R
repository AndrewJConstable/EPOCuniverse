...
#  2.1 set up stage-polygon matrices
    ...
    if(StageQtyInPolygon>0){
       ...
       if (!is.null(Mortality)) {
                 StageQtyInPolygon<-StageQtyInPolygon - sum(Mortality[(is.element(Mortality[,1],pn) &  # fixed mortality from activities
                 ...
                 if(Records.Mort>0) M.mort <- (-log(1-MortStage[,4] /StageQtyInPolygon)) else M.mort<-0
                  } else M.mort<-0
       ...
       if (!is.null(Emigration)) {
                 ...
                 M.Em  <- if(Records.Em>0) (-log(1-EmStage[,5]/StageQtyInPolygon)) else 0
