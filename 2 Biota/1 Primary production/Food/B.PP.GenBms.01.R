################################################################################
# PPgenBMS element class for all EPOC Elements
# S4
...
# Extend base class
setClass("PPgenBMS", contains="Biota")
	
setMethod("initialize", signature(.Object="PPgenBMS"),
    function(.Object, universe, dataPath, ...) {
        ...
        for (pn in 1:getSlot(.Object, "polygonsN")){
            bmsPn <- sum(initAbund[pn] * stage$StageStr[,pn] * stage$StageSize[,pn])
...
setMethod("initialiseReplicate", signature(element="PPgenBMS", universe="Universe"),
    function(element, universe) {

		scenario <- getScenario(universe)
		doPrintFinal(element, TRUE)

FoodElement<-getSlot(element,"epocAttributes")
FoodElementID<-FoodElement$Food.relatedElements

Food<-getState(universe$modules[[FoodElementID[1,1]]][[FoodElementID[1,2]]], "PolygonEnv")

   elemState <- getState(element)
    elemState$Abundance$mass <- Food
...
setMethod("initialiseTransition", signature(element="PPgenBMS", universe="Universe"),
    function(element, universe) {
