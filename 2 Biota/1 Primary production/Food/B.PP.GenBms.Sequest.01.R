# Create S4 method 'sequest'
#B.PP.GenBms.Sequest.01<-function(

# mortality function for phytoplankton biomass

if (!isGeneric("sequest"))
setGeneric("sequest", function(element, universe) standardGeneric("sequest"))
setMethod("sequest", signature(element="PPgenBMS", universe="Universe"),
    function(
		element,
		...
		)
	{
		...
		for (pn in 1:getSlot(element, "polygonsN")) {   # from origin polygons
			for (st in 1:getState(element, "StageN")) {
				...
				Died<-getState(element, "Abundance")$num.ind[pn] * getState(element, "Stage")[st,pn] * (1-exp(-NatMort))
				...
				mortality <- c(mortality,
					 ...
					 ,getState(element, "Cond.S")[st,pn] 			#  5.  stage size
