...
setMethod("advanceStageNumWtdBMS", signature(element="GET", universe="Universe"),
		...
		)     
    {
      ...
      FreqBySizeAtStage<-elemState$Stage[,pn] * elemState$Cond.S[,pn]
      elemState$Stage[,pn] <- elemState$Stage[,pn]%*%dSet$TransferStages
      NotZero<- elemState$Stage[,pn]>0
      elemState$Cond.S[NotZero,pn] <- (FreqBySizeAtStage%*%dSet$TransferStages)/elemState$Stage[NotZero,pn]
		  } # end pn
