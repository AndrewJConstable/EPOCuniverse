...
setMethod("migrateSetup", "GET",
    ...
    { 
      ...
      # loop through stages to modify matrix according to the following routine

      Nstage<-length(ptSA[[1]]$dset)

     for (st in 1:Nstage){

        ptSA[[1]]$dset[[st]] <- ptSA[[1]]$dset[[st]] * adj
    
        # 2. for each origin polygon (rows) determine fraction of
        #    stage class abundance going to destination cells using Baranov equation
        for (pn in 1:getSlot(element, "polygonsN")){
            TotalRate <- sum(ptSA[[1]]$dset[[st]][pn,])
            if(TotalRate>0) ptSA[[1]]$dset[[st]][pn,] <- ptSA[[1]]$dset[[st]][pn,] / TotalRate*(1-exp(-TotalRate))
        } # end pn
       } # end st
